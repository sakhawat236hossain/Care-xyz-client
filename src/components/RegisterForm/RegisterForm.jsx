...
const RegisterForm = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!passwordRegex.test(password)) {
      toast.error("Password must be 6+ chars with uppercase, lowercase & a number!");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (loginResult?.ok) {
          ...
          toast.error("Registration done, but login failed. Please login manually.");
          router.push("/login");
        }
      ...
      <form onSubmit={handleRegister} className="max-w-md w-full bg-white dark:bg-zinc-900 p-8 rounded-3xl shadow-xl border border-zinc-100 dark:border-zinc-800">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" name="name" placeholder="John Doe" className="w-full px-5 py-3 mt-1 rounded-xl border border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700 outline-none focus:border-blue-500 transition" />
          </div>

          <div>
            <label className="text-xs font-bold text-zinc-500 ml-1">Email Address</label>
            <input type="email" name="email" placeholder="example@mail.com" className="w-full px-5 py-3 mt-1 rounded-xl border border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700 outline-none focus:border-blue-500 transition" />
          </div>

          <div>
            <label className="text-xs font-bold text-zinc-500 ml-1">Phone Number</label>
            <input type="tel" name="phone" placeholder="+880 1XXX XXXXXX" className="w-full px-5 py-3 mt-1 rounded-xl border border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700 outline-none focus:border-blue-500 transition" />
          </div>

          <div>
            <label className="text-xs font-bold text-zinc-500 ml-1">Profile Image</label>
            <input type="file" name="image" accept="image/*" className="w-full px-5 py-2.5 mt-1 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition" />
          </div>
