...
export default function DashboardLayout({ children }) {
  ...
  const SidebarContent = () => (
    <>
      <div className="p-8 flex justify-between items-center">
        ...
        <button className="md:hidden p-2 text-zinc-500" onClick={() => setIsSidebarOpen(false)}>
          <X size={24} />
        </button>
      </div>
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] dark:bg-zinc-950 overflow-x-hidden">
      {/* --- Desktop Sidebar --- */}
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 md:hidden backdrop-blur-sm transition-opacity" onClick={() => setIsSidebarOpen(false)} />
      )}

      {/* --- Mobile Drawer --- */}
      <aside className={`fixed inset-y-0 left-0 w-72 bg-white dark:bg-zinc-900 z-[60] md:hidden transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <SidebarContent />
      </aside>
      ...
      <main className="flex-1 w-full flex flex-col min-w-0 overflow-hidden">
         {/* Top Header */}
         <header className="h-20 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-6 md:px-8 sticky top-0 z-40">
            <div className="flex items-center gap-4">
             
              <button className="md:hidden p-2 bg-zinc-100 dark:bg-zinc-800 rounded-lg text-zinc-600 dark:text-zinc-300" onClick={() => setIsSidebarOpen(true)}>
                <Menu size={20} />
              </button>
         ...
         {/* Content container with horizontal scroll protection */}
